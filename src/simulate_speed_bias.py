"""Show the steady tracking error caused by a persistent speed bias."""
...
def main() -> None:
    dt = 0.02
    speed_scale = 0.85  # The real robot receives only 85% of the requested forward speed.
    ...
    controller = ProportionalTrackingController(heading_gain=2.5, distance_gain=0.8)
    ...
    for index in range(len(time)):
        ...
        actual_v = speed_scale * commanded_v
        robot.step(actual_v, angular_velocity, dt)

    figure, (path_axis, error_axis) = plt.subplots(1, 2, figsize=(12, 5))
    path_axis.plot(ref_x, ref_y, "--", label="reference circle")
    path_axis.plot(actual_x, actual_y, label="P-controlled robot (85% speed)")
    path_axis.set_aspect("equal", adjustable="box")
    path_axis.set_xlabel("x (m)")
    path_axis.set_ylabel("y (m)")
    path_axis.set_title("P Control with a Persistent Speed Bias")
    ...
    error_axis.set_title("P Control Cannot Remove the Persistent Error")
    error_axis.grid(True, alpha=0.3)

    figure.tight_layout()
    output_path = Path("outputs/p_control_with_speed_bias.png")
